...
class Miner {
 	...
 	*/      
	public function logverbose($type,$data=NULL) {
		// Create log file
		$filename = "log.log";
	...
	function run() {
		if($this->configarray==NULL) {
			return "Problem with XML. Check and restart.\n";
		}
	...
	/**
 	* Outputs structured XML from html source
	...
	function outputXML() {
		...
		if($this->elements==NULL) {
			$this->logverbose("Element Check Failed",print_r($this->elements,true));
			return "No elements to parse. Check config XML.";
		}
		
		$html = $this->getSource();
		$tidy = tidy_parse_string($html)->html()->value;
		$querypathprep = $this->qpclean($tidy);
		$searchqp = htmlqp($querypathprep,'body');
	...
	/**
 	* Outputs structured XML from html source
 	* Used when data has dependencies
 	* i.e. structured/nested data
 	*/
	function outputDependentXML() {
		// Requires elements
		if($this->elements==NULL) {
			$this->logverbose("Element Check Failed",print_r($this->elements,true));
			return "No elements to parse. Check config XML.";
		...
		$html = $this->getSource();
		$tidy = tidy_parse_string($html)->html()->value;
		$querypathprep = $this->qpclean($html);
		$searchqp = htmlqp($querypathprep,'body');
		...
		// $type is an alternative of HTML or text
		$container = $searchqp->branch($this->configarray['element_head']);
		$this->logverbose("Number of results",count($container));
		$container_count = 0;
		foreach($container as $item) {
			if($item->branch($input[0])->$type[0]()!=NULL) {
				$container_count++;
				$this->logverbose("---------Result " . $container_count . "--------");
				$increment_head = "result" . $container_count;
				$head = $xml_output->addChild($increment_head);
				for ($i = 0; $i < count($input); $i++) {
					if($item->branch($input[$i])->$type[$i]()!=NULL) {
						$pulled = $item->branch($input[$i])->$type[$i]();
						$this->logverbose("Item---------",$pulled);
						$head->$output[$i] = $pulled;
					} else {
						$this->logverbose("Item---------",$output[$i] . " Empty\n");
						$head->$output[$i] = "EMPTY";
					}
				}
			}
